...
static char* em_sapi_headers_status_line(int code) {
    switch (code) {
        case 200: return "OK";
...
sapi_header_struct* em_sapi_headers_status(sapi_headers_struct* all, zend_llist_position* position) {
    ...
    /* Status: sent by script is authority */
    do {
        if (!one) {
            break;
        }
    ...
    if (all->http_response_code < 200 ||
        all->http_response_code > 505) {
        all->http_response_code = 200;
    }

    all->http_status_line = estrdup(
        em_sapi_headers_status_line(
            all->http_response_code));
